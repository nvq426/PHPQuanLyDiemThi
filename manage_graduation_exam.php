...
// Xử lý tạo lịch thi tốt nghiệp
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_graduation_schedule'])) {
    ...
    $stmt = $db->prepare("INSERT INTO ExamSchedules (subjectID, roomID, teacherID1, teacherID2, examDate, examShift, duration) VALUES (:subjectID, :roomID, :teacherID1, :teacherID2, :examDate, 'Sáng', 120)");
...
// Xử lý nhập điểm thi tốt nghiệp
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['enter_graduation_result'])) {
    $studentID = $_POST['student'];
    $subjectID = $_POST['subject'];
    $score = $_POST['score'];

    $stmt = $db->prepare("INSERT INTO ExamResults (studentID, subjectID, score, semester, isGraduationExam) VALUES (:studentID, :subjectID, :score, 'Graduation', 1)");
...
// Xử lý xuất học bạ dưới dạng CSV
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['generate_transcript'])) {
    $studentID = $_POST['student'];
    $stmt = $db->prepare("SELECT * FROM Transcripts WHERE studentID = :studentID");
    $stmt->bindValue(':studentID', $studentID, SQLITE3_INTEGER);
    $transcript = $stmt->execute()->fetchArray(SQLITE3_ASSOC);

    $stmt = $db->prepare("SELECT ExamResults.*, Subjects.subjectName 
                          FROM ExamResults 
                          JOIN Subjects ON ExamResults.subjectID = Subjects.subjectID 
                          JOIN Transcript_ExamResult ON ExamResults.resultID = Transcript_ExamResult.resultID 
                          WHERE Transcript_ExamResult.transcriptID = :transcriptID");
    $stmt->bindValue(':transcriptID', $transcript['transcriptID'], SQLITE3_INTEGER);
    ...
    header('Content-Disposition: attachment; filename="transcript_' . $studentID . '.csv"');
    header('Cache-Control: no-cache, no-store, must-revalidate');
    header('Pragma: no-cache');
    header('Expires: 0');

    // Ghi thông tin sinh viên
    fputcsv($output, ['Họ tên', $studentInfo['fullName']]);
    fputcsv($output, ['Mã SV', $studentID]);
    fputcsv($output, ['Điểm trung bình', $transcript['averageScore']]);
    fputcsv($output, []); // Dòng trống
    fputcsv($output, ['Môn học', 'Điểm', 'Học kỳ']); // Tiêu đề bảng điểm

    // Ghi chi tiết điểm số
    while ($row = $results->fetchArray(SQLITE3_ASSOC)) {
        fputcsv($output, [$row['subjectName'], $row['score'], $row['semester']]);
    }
...
$subjects = $db->query("SELECT * FROM Subjects WHERE subjectName LIKE '%Tốt nghiệp%'");
...
<head>
    ...
    <title>Quản lý Thi Tốt nghiệp</title>
    ...
    <style>
        ...
        form button:hover, .create-btn:hover {
            background-color: #0277BD;
        }
        .popup {
            display: none;
...
<body>
    ...
    <div class="content">
        <h1>Quản lý Thi Tốt nghiệp</h1>
        <h2>Lập Lịch thi Tốt nghiệp</h2>
        <button class="create-btn" data-popup="create-graduation-schedule-popup">Tạo Lịch thi Tốt nghiệp</button>

        <h2>Nhập Điểm thi Tốt nghiệp</h2>
        <button class="create-btn" data-popup="enter-graduation-result-popup">Nhập Điểm</button>

        <h2>Xuất Học bạ</h2>
        <form action="manage_graduation_exam.php" method="post">
            ...
            <input type="hidden" name="generate_transcript" value="1">
            <button type="submit">Xuất Học bạ</button>
        </form>
    </div>

    <!-- Popup Tạo Lịch thi Tốt nghiệp -->
    <div id="create-graduation-schedule-popup" class="popup">
        <h2>Tạo Lịch thi Tốt nghiệp</h2>
        <form action="manage_graduation_exam.php" method="post">
            <label for="subject">Môn thi:</label>
            <input list="subjects-graduation" name="subject" placeholder="Chọn môn thi" required>
            <datalist id="subjects-graduation">
                <?php 
                $subjects->reset();
            ...
            <input list="rooms-graduation" name="room" placeholder="Chọn phòng thi" required>
            <datalist id="rooms-graduation">
                <?php 
                $rooms->reset();
            ...
            <input list="teachers-graduation-1" name="teacher1" placeholder="Chọn giáo viên 1" required>
            <datalist id="teachers-graduation-1">
                <?php 
                $teachers->reset();
            ...
            <input list="teachers-graduation-2" name="teacher2" placeholder="Chọn giáo viên 2" required>
            <datalist id="teachers-graduation-2">
                <?php 
                $teachers->reset();
                while ($teacher = $teachers->fetchArray(SQLITE3_ASSOC)): ?>
                    <option value="<?php echo htmlspecialchars($teacher['teacherID']); ?>" data-label="<?php echo htmlspecialchars($teacher['fullName']); ?>"><?php echo htmlspecialchars($teacher['fullName']); ?></option>
            ...
            <input type="hidden" name="create_graduation_schedule" value="1">
            <button type="submit">Tạo</button>
            <button type="button" class="close-btn" data-popup="create-graduation-schedule-popup">Đóng</button>
        </form>
    </div>
    <div id="create-graduation-schedule-popup-overlay" class="overlay" data-popup="create-graduation-schedule-popup"></div>

    <!-- Popup Nhập Điểm thi Tốt nghiệp -->
    <div id="enter-graduation-result-popup" class="popup">
        <h2>Nhập Điểm thi Tốt nghiệp</h2>
        <form action="manage_graduation_exam.php" method="post">
            <label for="student">Sinh viên:</label>
            <input list="students-graduation" name="student" placeholder="Chọn sinh viên" required>
            <datalist id="students-graduation">
                <?php 
                $students->reset();
                while ($student = $students->fetchArray(SQLITE3_ASSOC)): ?>
                    <option value="<?php echo htmlspecialchars($student['studentID']); ?>" data-label="<?php echo htmlspecialchars($student['fullName']); ?>"><?php echo htmlspecialchars($student['fullName']); ?></option>
            ...
            <label for="subject">Môn thi:</label>
            <input list="subjects-graduation-enter" name="subject" placeholder="Chọn môn thi" required>
            <datalist id="subjects-graduation-enter">
                <?php 
                $subjects->reset();
                while ($subject = $subjects->fetchArray(SQLITE3_ASSOC)): ?>
                    <option value="<?php echo htmlspecialchars($subject['subjectID']); ?>" data-label="<?php echo htmlspecialchars($subject['subjectName']); ?>"><?php echo htmlspecialchars($subject['subjectName']); ?></option>
            ...
            <input type="hidden" name="enter_graduation_result" value="1">
            <button type="submit">Nhập</button>
            <button type="button" class="close-btn" data-popup="enter-graduation-result-popup">Đóng</button>
        </form>
    </div>
    <div id="enter-graduation-result-popup-overlay" class="overlay" data-popup="enter-graduation-result-popup"></div>
