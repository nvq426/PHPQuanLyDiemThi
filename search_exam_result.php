...
// Xử lý tra cứu kết quả thi
$whereClause = [];
$filters = [];
$whereClause[] = "ExamResults.studentID = :studentID";
$filters[':studentID'] = $_SESSION['userID'];

if (isset($_GET['subject_filter']) && $_GET['subject_filter'] != '') {
    $whereClause[] = "ExamResults.subjectID = :subjectID";
    $filters[':subjectID'] = (int)$_GET['subject_filter'];
}
if (isset($_GET['semester_filter']) && $_GET['semester_filter'] != '') {
    $whereClause[] = "ExamResults.semester = :semester";
    $filters[':semester'] = $_GET['semester_filter'];
}
...
<body>
    ...
    <div class="content">
        ...
        <div class="filters">
            <form class="filter-group" action="search_exam_result.php" method="get">
                ...
                <input type="text" name="semester_filter" placeholder="Lọc theo Học kỳ (VD: 2025-1)" value="<?php echo isset($_GET['semester_filter']) ? htmlspecialchars($_GET['semester_filter']) : ''; ?>">
        ...
        <table>
            ...
            <tbody>
                ...
                <?php if (!$hasResults): ?>
                    <tr>
                        <td colspan="3">Không có dữ liệu kết quả thi phù hợp với điều kiện tra cứu.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
