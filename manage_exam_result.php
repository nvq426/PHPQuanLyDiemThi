...
if (!isset($_SESSION['role']) || !in_array($_SESSION['role'], ['education_office', 'teacher'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['enter_result'])) {
    ...
    $stmt = $db->prepare("INSERT INTO ExamResults (studentID, subjectID, score, semester) VALUES (:studentID, :subjectID, :score, :semester)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_result'])) {
    ...
    $stmt = $db->prepare("UPDATE ExamResults SET score = :score, semester = :semester WHERE resultID = :resultID");
    $stmt->bindValue(':resultID', $resultID, SQLITE3_INTEGER);
    $stmt->bindValue(':score', $score, SQLITE3_FLOAT);
    $stmt->bindValue(':semester', $semester, SQLITE3_TEXT);
    $stmt->execute();
}
...
if (isset($_GET['student_filter']) && $_GET['student_filter'] != '') {
    $whereClause[] = "studentID = :studentID";
    $filters[':studentID'] = (int)$_GET['student_filter'];
}

$query = "SELECT ExamResults.*, Students.fullName, Subjects.subjectName 
          FROM ExamResults 
          JOIN Students ON ExamResults.studentID = Students.studentID 
...
<body>
    ...
    <div class="header">
        <h2><?php echo $_SESSION['role'] === 'education_office' ? 'Phòng Giáo vụ' : 'Giáo viên'; ?></h2>
        <ul>
            <?php if ($_SESSION['role'] === 'education_office'): ?>
                <li>
    ...
    <div class="content">
        ...
        <div class="filters">
            <form class="filter-group" action="manage_exam_result.php" method="get">
                <input list="subjects" name="subject_filter" placeholder="Lọc theo Môn học" value="<?php echo isset($_GET['subject_filter']) ? htmlspecialchars($_GET['subject_filter']) : ''; ?>">
                ...
                <input list="students" name="student_filter" placeholder="Lọc theo Sinh viên" value="<?php echo isset($_GET['student_filter']) ? htmlspecialchars($_GET['student_filter']) : ''; ?>">
            ...
            <button class="create-btn" data-popup="enter-result-popup">Nhập Điểm</button>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php if (!$result): ?>
                    <tr>
                        <td colspan="5">Không có dữ liệu kết quả thi.</td>
                    </tr>
                <?php else: ?>
                    <?php while ($row = $result->fetchArray(SQLITE3_ASSOC)): ?>
                        <tr>
                            ...
                            <td>
                                <a href="#" class="create-btn" data-popup="edit-result-popup-<?php echo htmlspecialchars($row['resultID']); ?>">Sửa</a>
                            </td>
                        </tr>

                        <!-- Popup Sửa Điểm -->
                        <div id="edit-result-popup-<?php echo htmlspecialchars($row['resultID']); ?>" class="popup">
                            <h2>Sửa Điểm thi</h2>
                            <form action="manage_exam_result.php" method="post">
                                <label for="score-<?php echo htmlspecialchars($row['resultID']); ?>">Điểm:</label>
                                <input type="number" id="score-<?php echo htmlspecialchars($row['resultID']); ?>" name="score" min="0" max="10" step="0.1" value="<?php echo htmlspecialchars($row['score']); ?>" required>

                                <label for="semester-<?php echo htmlspecialchars($row['resultID']); ?>">Học kỳ:</label>
                                <input type="text" id="semester-<?php echo htmlspecialchars($row['resultID']); ?>" name="semester" value="<?php echo htmlspecialchars($row['semester']); ?>" placeholder="VD: 2025-1" required>

                                <input type="hidden" name="result_id" value="<?php echo htmlspecialchars($row['resultID']); ?>">
                                <input type="hidden" name="edit_result" value="1">
                                <button type="submit">Lưu</button>
                                <button type="button" class="close-btn" data-popup="edit-result-popup-<?php echo htmlspecialchars($row['resultID']); ?>">Đóng</button>
                            </form>
                        </div>
                        <div id="edit-result-popup-<?php echo htmlspecialchars($row['resultID']); ?>-overlay" class="overlay" data-popup="edit-result-popup-<?php echo htmlspecialchars($row['resultID']); ?>"></div>
                    <?php endwhile; ?>
                <?php endif; ?>
            </tbody>
        </table>
    ...
    <!-- Popup Nhập Điểm -->
    <div id="enter-result-popup" class="popup">
        <h2>Nhập Điểm thi</h2>
        <form action="manage_exam_result.php" method="post">
            <label for="student">Sinh viên:</label>
            <input list="students-enter" name="student" placeholder="Chọn sinh viên" required>
            <datalist id="students-enter">
                <?php 
                $students->reset();
                while ($student = $students->fetchArray(SQLITE3_ASSOC)): ?>
                    <option value="<?php echo htmlspecialchars($student['studentID']); ?>" data-label="<?php echo htmlspecialchars($student['fullName']); ?>"><?php echo htmlspecialchars($student['fullName']); ?></option>
            ...
            <input list="subjects-enter" name="subject" placeholder="Chọn môn học" required>
            <datalist id="subjects-enter">
                <?php 
                $subjects->reset();
                while ($subject = $subjects->fetchArray(SQLITE3_ASSOC)): ?>
                    <option value="<?php echo htmlspecialchars($subject['subjectID']); ?>" data-label="<?php echo htmlspecialchars($subject['subjectName']); ?>"><?php echo htmlspecialchars($subject['subjectName']); ?></option>
            ...
            <input type="text" id="semester" name="semester" placeholder="VD: 2025-1" required>

            <input type="hidden" name="enter_result" value="1">
            <button type="submit">Nhập</button>
            <button type="button" class="close-btn" data-popup="enter-result-popup">Đóng</button>
        </form>
    </div>
    <div id="enter-result-popup-overlay" class="overlay" data-popup="enter-result-popup"></div>
